...
class TestBaseRepositoryDataProcessing:
    ...
    def test_preprocess_data_with_psycopg_adapters(self, mock_connection):
        """Test preprocessing data with psycopg adapters enabled."""
        repo = BaseRepository(
            db_connection=mock_connection,
            table_name="test_table",
            model_class=SampleJSONModel,
        ...
        processed = repo._preprocess_data(test_data)

        # Non-JSON fields should remain unchanged
        assert processed["name"] == "test_item"
        assert processed["value"] == 42

        # JSON fields should be wrapped with Json()
        assert isinstance(processed["metadata"], Json)
        assert isinstance(processed["tags"], Json)
        assert isinstance(processed["settings"], Json)

        # Verify the wrapped values
        assert processed["metadata"].obj == {"key": "value", "number": 123}
        assert processed["tags"].obj == ["tag1", "tag2"]
        assert processed["settings"].obj == {"theme": "dark"}
    ...
    def test_preprocess_data_no_json_fields(self, non_json_repo):
        ...
        test_data = {"name": "test_item", "active": True}
    ...
    def test_preprocess_data_serialization_error(self, json_repo):
        ...
        test_data = {"name": "test_item", "value": 42, "metadata": {"bad_data": NonSerializable()}}
    ...
    def test_data_immutability(self, json_repo):
        ...
        original_data = {"name": "test_item", "value": 42, "metadata": {"key": "value"}, "tags": ["tag1", "tag2"]}
    ...
    def test_preprocessing_logging(self, mock_logger, json_repo):
        ...
        test_data = {"name": "test_item", "metadata": {"key": "value"}, "tags": ["tag1"]}
    ...
    def test_postprocessing_error_logging(self, mock_logger, json_repo):
        ...
        test_data = {"name": "test_item", "metadata": "invalid json {", "tags": '["valid"]'}
