...
class TestBaseRepositoryCRUDWithJSON:
    ...
    async def test_create_without_json_fields(self, mock_connection):
        ...
        non_json_repo = BaseRepository(
            db_connection=mock_connection, table_name="simple_table", model_class=NonJSONModel, primary_key="id"
        )

        model = NonJSONModel(name="test", active=True)

        db_result = {"id": model.id, "name": "test", "active": True}
    ...
    async def test_create_with_explicit_json_fields_config(self, mock_connection):
        ...
        sample = MixedFieldModel(name="test", config={"key": "value", "number": 123}, simple_list=["item1", "item2"])
    ...
    async def test_create_bulk_with_different_batch_sizes(self, json_repo):
        ...
        models = [
            JSONTestModel(name=f"test_item_{i}", value=i, metadata={"index": i}, tags=[f"tag_{i}"]) for i in range(5)
        ]
    ...
    async def test_create_bulk_json_serialization_error(self, json_repo):
        ...
        models = [
            JSONTestModel(name="test_item_1", value=42, metadata={"good_data": "value"}, tags=["tag1"]),
            JSONTestModel(
                name="test_item_2",
                value=43,
                metadata={"bad_data": NonSerializable()},  # This will fail serialization
    ...
    async def test_create_bulk_database_transaction_error(self, json_repo):
        ...
        models = [JSONTestModel(name="test_item", value=42, metadata={"key": "value"}, tags=["tag1"])]
    ...
    async def test_create_bulk_without_json_fields(self, mock_connection):
        ...
        models = [NonJSONModel(name=f"test_{i}", active=i % 2 == 0) for i in range(3)]

        db_results = [{"id": model.id, "name": model.name, "active": model.active} for model in models]
    ...
    async def test_create_bulk_preserves_original_models(self, json_repo):
        ...
        models = [JSONTestModel(name="test_item", value=42, metadata={"key": "value"}, tags=["tag1", "tag2"])]
    ...
    async def test_get_by_id_without_json_fields(self, mock_connection):
        ...
        test_id = uuid4()
        db_result = {"id": test_id, "name": "test", "active": True}
    ...
    async def test_update_without_json_fields(self, mock_connection):
        ...
        db_result = {"id": test_id, "name": "updated_test", "active": False}
    ...
    async def test_update_preserves_original_data(self, json_repo, sample_model):
        ...
        update_data = {"metadata": {"key": "value"}, "tags": ["tag1", "tag2"]}
    ...
    async def test_update_logging_behavior(self, mock_logger, json_repo, sample_model):
        ...
        update_data = {"metadata": {"key": "updated_value"}, "tags": ["new_tag"]}
