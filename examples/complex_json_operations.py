...
class ShippingAddress(BaseModel):
    ...
    coordinates: Optional[Dict[str, float]] = None  # {"lat": 0.0, "lon": 0.0}
...
class OrderRepository(BaseRepository[Order, UUID]):
    ...
    async def find_by_customer(self, customer_id: UUID) -> List[Order]:
        ...
        async with self.db_connection.cursor() as cur:
            ...
            return [
                self.model_class(**self._postprocess_data(dict(row)))
                for row in rows
            ]
    ...
    async with db.connection() as conn:
        ...
        try:
            async with tx_manager.transaction() as tx_conn:
                ...
                updated_tx = await tx_order_repo.update_order_status(
                    created_tx.id, 
                    "confirmed"
                )
        ...
        analytics = Analytics(
            ...
            metrics={
                "orders": [
                    {"timestamp": datetime.now().isoformat(), "value": len(customer_orders)}
                ],
                "revenue": [
                    {"timestamp": datetime.now().isoformat(), "value": float(sum(o.total_amount for o in customer_orders))}
                ]
            },
            aggregates={
                "total_orders": len(customer_orders),
                "avg_order_value": float(sum(o.total_amount for o in customer_orders) / len(customer_orders)) if customer_orders else 0
            },
        ...
        print(f"  Created analytics entry for customer")
